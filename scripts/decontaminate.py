...
MULTI_Rotifers=[]
MULTI_Algae=[]
MULTI_Extraction_Contaminants=[]
MULTI_PCR_Contaminants=[]
MULTI_Spikein=[]
...
if os.path.exists("Decontaminated_zOTU_table.txt"):
	os.remove("Decontaminated_zOTU_table.txt")
if os.path.exists("Decontaminated_OTU_Table.txt"):
	os.remove("Decontaminated_OTU_Table.txt")
if os.path.exists("Table_with_classes.txt"):
	os.remove("Table_with_classes.txt")
if os.path.exists("Statistics_table.txt"):
	os.remove("Statistics_table.txt")
...
for line in BLANKS:
	...
	elif len(LINE_BLANK) == 2 and LINE_BLANK[1] == "blank_algae":
		Blank_Algae.append(LINE_BLANK[0])
	elif len(LINE_BLANK) == 2 and LINE_BLANK[1] == "blank_rotifers":
		Blank_Rotifers.append(LINE_BLANK[0])
...
if len(Blank_Algae) >= 1:
		print('\nI am going to use %s of Algae blanks:' %len(Blank_Algae))
		for item in Blank_Algae:
			print(item, end=", ")
elif len(Blank_Algae) == 0:
	Blank_Algae.append("There is no Algae blanks")
	print("There is no Algae blanks")
print()


if len(Blank_Rotifers) >= 1:
		print('\nI am going to use %s of Rotifers blanks:' %len(Blank_Rotifers))
		for item in Blank_Rotifers:
			print(item, end=", ")
elif len(Blank_Rotifers) == 0:
	Blank_Rotifers.append("There is no Rotifers blanks")
	print("There is no Rotifers blanks")
print()
...
for row_no in range(1, len(TABLE[:-1])):
		...
		###print(row_no, col_no, TABLE[row_no][col_no])###
			if TABLE[0][col_no] in Blank_PCR:
				...
				if max_r_extr < relabund:
					max_r_extr = relabund
			elif TABLE[0][col_no] in Blank_Algae:
				if max_r_algae < relabund:
					max_r_algae = relabund
			elif TABLE[0][col_no] in Blank_Rotifers:
				if max_r_rotifers < relabund:
					max_r_rotifers = relabund
			else:
	...
	elif max_r_real > max(max_r_extr, max_r_PCR, max_r_algae, max_r_rotifers) * float(ThresholdA):
		Decision = "Symbiont"
	elif max(max_r_algae,max_r_rotifers) > max(max_r_extr, max_r_PCR)  * float(ThresholdA):##################
		if max_r_algae>max_r_rotifers:
			Decision = "Algae"
		else:
			Decision = "Rotifers"
...
for col_no in range(5,COL_NO-1):
	...
	for row_no in range(1,len(TABLE)):
		...
		if "Algae" in TABLE[row_no][-1]:
			algae += int(TABLE[row_no][col_no])
		if "Rotifers" in TABLE[row_no][-1]:
			rotifers += int(TABLE[row_no][col_no])
...
Statistics.append(ALGAE)
Statistics.append(ROTIFERS)
...
### 9ht row - sum of 'Algae' reads
### 10ht row - sum of 'Rotifers' reads
### 11th row - sum of reads in the library
...
ALG = ["%_of_all_Algae_reads_in_lib"]
ROT = ["%_of_all_Rotifers_reads_in_lib"]
...
for col_no in range(1, len(Statistics[0])):
	...
	for row_no in range(1, len(Statistics[:-1])):
		...
		Alg = int(Statistics[8][col_no])/int(Statistics[-1][col_no]) * 100		
		Rot = int(Statistics[9][col_no])/int(Statistics[-1][col_no]) * 100		
...
#tu są jaja
	...
	if "LIPIEC_16S_v4_T_KE008_B02" == Statistics[0][col_no]:
		MULTI_Threshold.append(float(ThresholdA))
		MULTI_Symbionts.append(Symbionts)
		MULTI_Rotifers.append(Rot)
		MULTI_Algae.append(Alg)
		MULTI_Extraction_Contaminants.append(Con_Ext)
		MULTI_PCR_Contaminants.append(Con_PCR)
		MULTI_Spikein.append(Extr_Spike)
...
Statistics.append(ALG)
Statistics.append(ROT)
...
for list0 in OTU_TAX:
	list0[0] = list0[0].lower()
...
with open("Multi_results.txt", "w") as multiFile:
	print(*["Threshold",*MULTI_Threshold],sep="\t", end="\n", file = multiFile)
	print(*["Spikein", *MULTI_Spikein],sep="\t", end="\n", file = multiFile)
	print(*["Extraction_contaminants", *MULTI_Extraction_Contaminants],sep="\t", end="\n", file = multiFile)
	print(*["PCR_contaminants",*MULTI_PCR_Contaminants],sep="\t", end="\n", file = multiFile)
	print(*["Algae",*MULTI_Algae],sep="\t", end="\n", file = multiFile)	
	print(*["Rotifers",*MULTI_Rotifers],sep="\t", end="\n", file = multiFile)
	print(*["Symbionts",*MULTI_Symbionts],sep="\t", end="\n", file = multiFile)		
